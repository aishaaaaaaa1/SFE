...
function navHtml(assetPrefix, arabicHref = null) {
  const h = assetPrefix + 'index.html';
  const p = (f) => (assetPrefix === '../' ? '' : 'pages/') + f;
  const logo = assetPrefix + 'assets/images/logo-fr.png';
  const arHref = arabicHref || p('ar/index.html');
  return `<header id="header">
  <div class="header-inner">
    <a href="${h}" class="logo">
      <img class="logo-img" src="${logo}" alt="Agence Urbaine de Dakhla – Oued Eddahab"/>
    </a>
    <nav>
      <div class="nav-item">
      ...
      <a href="${arHref}" class="lang-btn">🇲🇦 العربية</a>
    </nav>
...
function mobileHtml(assetPrefix, arabicHref = null) {
  const h = assetPrefix + 'index.html';
  const p = (f) => (assetPrefix === '../' ? '' : 'pages/') + f;
  const arHref = arabicHref || p('ar/index.html');
  ...
  <a href="${arHref}" style="color:var(--gold-light);font-weight:600">🇲🇦 العربية</a>
</div>`;
}

function navHtmlArabic(assetPrefix, frenchHref) {
  const p = (f) => f;
  const logo = assetPrefix + 'assets/images/logo-ar.png';
  return `<header id="header">
  <div class="header-inner">
    <a href="${p('index.html')}" class="logo">
      <img class="logo-img" src="${logo}" alt="الوكالة الحضرية للداخلة وادي الذهب"/>
    </a>
    <nav>
      <div class="nav-item">
        <span class="nav-link">التقديم <svg class="arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M6 9l6 6 6-6"/></svg></span>
        <div class="dropdown">
          <a href="${p('presentation-creation.html')}">إحداث الوكالة</a>
          <a href="${p('presentation-missions.html')}">المهام والاختصاصات</a>
          <a href="${p('presentation-champs.html')}">مجال التدخل الترابي</a>
          <a href="${p('presentation-organisation.html')}">التنظيم الهيكلي</a>
          <a href="${p('presentation-responsables.html')}">مسؤولو الوكالة</a>
        </div>
      </div>
      <div class="nav-item">
        <span class="nav-link">التدبير الحضري <svg class="arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M6 9l6 6 6-6"/></svg></span>
        <div class="dropdown">
          <a href="${p('gestion-procedures.html')}">مساطر الترخيص</a>
          <a href="${p('gestion-assistance.html')}">المواكبة التقنية والمعمارية</a>
          <a href="${p('gestion-controle.html')}">مراقبة الأوراش</a>
        </div>
      </div>
      <div class="nav-item">
        <span class="nav-link">التخطيط <svg class="arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M6 9l6 6 6-6"/></svg></span>
        <div class="dropdown">
          <a href="${p('planif-documents.html')}">وثائق التعمير</a>
          <a href="${p('planif-etudes.html')}">الدراسات العمرانية والمعمارية</a>
        </div>
      </div>
      <div class="nav-item">
        <span class="nav-link">الخدمات الرقمية <svg class="arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M6 9l6 6 6-6"/></svg></span>
        <div class="dropdown">
          <a href="${p('service-preinstruction.html')}">الدراسة القبلية الإلكترونية</a>
          <a href="${p('service-enote.html')}">E-Note</a>
          <a href="${p('service-reclamation.html')}">الشكايات الإلكترونية</a>
          <a href="${p('service-demande-info.html')}">طلب المعلومات</a>
          <a href="${p('service-taamir.html')}">وثائق التعمير (Taamir)</a>
          <a href="${p('service-bod.html')}">مكتب الضبط الرقمي</a>
        </div>
      </div>
      <div class="nav-item">
        <span class="nav-link">النصوص التنظيمية <svg class="arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M6 9l6 6 6-6"/></svg></span>
        <div class="dropdown">
          <a href="${p('reg-dahirs.html')}">الظهائر</a>
          <a href="${p('reg-decrets.html')}">المراسيم</a>
          <a href="${p('reg-circulaires.html')}">الدوريات</a>
          <a href="${p('reg-arretes.html')}">القرارات</a>
        </div>
      </div>
      <a href="${p('contact.html')}" class="nav-link">اتصل بنا</a>
      <a href="${p('service-preinstruction.html')}" class="nav-link header-cta">الدراسة القبلية الإلكترونية</a>
      <a href="${frenchHref}" class="lang-btn">🇫🇷 Français</a>
    </nav>
    <div class="burger" onclick="toggleMenu()"><span></span><span></span><span></span></div>
  </div>
</header>`;
...
function mobileHtmlArabic(assetPrefix, frenchHref) {
  const p = (f) => f;
  return `<div class="mobile-menu" id="mobileMenu">
  <button type="button" class="mobile-close" onclick="toggleMenu()">✕</button>
  <a href="${p('index.html')}">الرئيسية</a>
  <a href="${p('presentation-creation.html')}">التقديم</a>
  <a href="${p('gestion-procedures.html')}">التدبير الحضري</a>
  <a href="${p('planif-documents.html')}">التخطيط الحضري</a>
  <a href="${p('service-preinstruction.html')}">الخدمات الرقمية</a>
  <a href="${p('reg-dahirs.html')}">النصوص التنظيمية</a>
  <a href="${p('contact.html')}">اتصل بنا</a>
  <a href="${frenchHref}" style="color:var(--gold-light);font-weight:600">النسخة الفرنسية ←</a>
</div>`;
}
  ...
  <div class="footer-grid">
    <div class="footer-brand">
      <a href="${h}" class="logo" style="display:inline-flex">
        <img class="logo-img logo-img--footer" src="${logo}" alt="Agence Urbaine de Dakhla – Oued Eddahab"/>
      </a>
...
function footerHtmlArabic(assetPrefix) {
  const p = (f) => f;
  const logo = assetPrefix + 'assets/images/logo-ar.png';
  return `<footer id="footer">
  <div class="footer-grid" dir="rtl">
    <div class="footer-brand">
      <a href="${p('index.html')}" class="logo" style="display:inline-flex">
        <img class="logo-img logo-img--footer" src="${logo}" alt="الوكالة الحضرية للداخلة وادي الذهب"/>
      </a>
      <p>الوكالة الحضرية للداخلة وادي الذهب مؤسسة عمومية في خدمة التنمية الحضرية المستدامة بالجهة.</p>
      <div class="footer-socials">
        <a class="social-btn" href="https://www.facebook.com/agencedakhla/" target="_blank" rel="noopener noreferrer" title="فيسبوك"><svg viewBox="0 0 24 24"><path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z"/></svg></a>
        <a class="social-btn" href="${p('galerie.html')}" title="معرض الصور"><svg viewBox="0 0 24 24"><rect x="2" y="2" width="20" height="20" rx="5"/><path d="M16 11.37A4 4 0 1112.63 8 4 4 0 0116 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg></a>
        <a class="social-btn" href="${p('actualites.html')}" title="الأخبار"><svg viewBox="0 0 24 24"><path d="M16 8a6 6 0 016 6v7h-4v-7a2 2 0 00-2-2 2 2 0 00-2 2v7h-4v-7a6 6 0 016-6z"/><rect x="2" y="9" width="4" height="12"/><circle cx="4" cy="4" r="2"/></svg></a>
      </div>
    </div>
    <div class="footer-col">
      <h4>روابط</h4>
      <ul class="footer-links">
        <li><a href="${p('presentation-creation.html')}">التقديم</a></li>
        <li><a href="${p('gestion-procedures.html')}">التدبير الحضري</a></li>
        <li><a href="${p('planif-documents.html')}">التخطيط</a></li>
        <li><a href="${p('service-preinstruction.html')}">الخدمات الرقمية</a></li>
        <li><a href="${p('reg-dahirs.html')}">النصوص التنظيمية</a></li>
        <li><a href="${p('contact.html')}">اتصل بنا</a></li>
        <li><a href="${p('avis-annonces.html')}">إعلانات</a></li>
      </ul>
    </div>
    <div class="footer-col">
      <h4>خدمات</h4>
      <ul class="footer-links">
        <li><a href="${p('service-preinstruction.html')}">الدراسة القبلية</a></li>
        <li><a href="${p('service-enote.html')}">E-Note</a></li>
        <li><a href="${p('service-bod.html')}">مكتب الضبط الرقمي</a></li>
        <li><a href="${p('service-reclamation.html')}">الشكايات الإلكترونية</a></li>
        <li><a href="${p('service-taamir.html')}">وثائق التعمير (Taamir)</a></li>
        <li><a href="${p('service-demande-info.html')}">طلب المعلومات</a></li>
      </ul>
    </div>
    <div class="footer-col">
      <h4>اتصال</h4>
      <div class="footer-contact">
        <div class="contact-item">
          <svg viewBox="0 0 24 24"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>
          <span>شارع محمد السادس، الداخلة – وادي الذهب، المغرب</span>
        </div>
        <div class="contact-item">
          <svg viewBox="0 0 24 24"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>
          <span>0528-89-78-01</span>
      ...
      <div style="margin-top:20px">
        <p style="font-size:12px;color:rgba(255,255,255,.4);margin-bottom:10px;font-weight:600;letter-spacing:.06em;text-transform:uppercase">النشرة الإخبارية</p>
        <div class="newsletter-form">
  ...
  <div class="footer-bottom" dir="rtl">
    <p>© 2026 الوكالة الحضرية للداخلة وادي الذهب. جميع الحقوق محفوظة.</p>
    <div class="footer-bottom-links">
      <a href="${p('plan-site.html')}">خريطة الموقع</a>
      <a href="${p('index.html')}">سياسة الخصوصية</a>
      <a href="${p('index.html')}">ولوجية</a>
    </div>
  </div>
</footer>`;
}

function breadcrumbHtml(items, ariaLabel = "Fil d'Ariane") {
  const parts = items.map(([label, href]) => {
    if (href) return `<a href="${href}">${label}</a>`;
    return `<span>${label}</span>`;
  });
  return `<nav class="breadcrumb" aria-label="${ariaLabel}">${parts.join('<span class="sep">/</span>')}</nav>`;
}

function toArabicLabel(label) {
  const map = {
    'Accueil': 'الرئيسية',
    ...
    'Contact': 'اتصل بنا',
    'Actualités': 'الأخبار',
    'Avis & Annonces': 'إعلانات',
    'Plan du site': 'خريطة الموقع',
    'Création': 'إحداث الوكالة',
    'Missions': 'المهام',
    'Champs d’action': 'مجال التدخل',
    ...
    'Documents': 'الوثائق',
    'Études': 'الدراسات',
    'Pré-instruction': 'الدراسة القبلية',
    'E-Note': 'E-Note',
    "Demandes d'information": 'طلبات المعلومات',
    'Réclamations': 'الشكايات',
    "Bureau d'Ordre Digital": 'مكتب الضبط الرقمي',
    'Documents en ligne (Taamir)': 'وثائق التعمير (Taamir)',
    'Dahirs': 'الظهائر',
    'Décrets': 'المراسيم',
    'Circulaires': 'الدوريات',
    'Arrêtés': 'القرارات',
    'Création de l’Agence': 'إحداث الوكالة',
    ...
    'Assistance technique et architecturale': 'المواكبة التقنية والمعمارية',
    'Contrôle de chantier': 'مراقبة الأوراش',
    'Documents d’urbanisme': 'وثائق التعمير',
    'Études urbanistiques et architecturales': 'الدراسات العمرانية والمعمارية',
    'Pré-instruction en ligne': 'الدراسة القبلية الإلكترونية',
    'E-Note de renseignements': 'E-Note لطلب المعلومات التعميرية',
    'Réclamation en ligne': 'الشكايات الإلكترونية',
    'Demande d’informations': 'طلب المعلومات',
    'Nous contacter': 'اتصل بنا',
    'Documents d’urbanisme en ligne (Taamir)': 'وثائق التعمير (Taamir)',
    'Bureau d’Ordre Digital': 'مكتب الضبط الرقمي',
    'Actualités': 'الأخبار',
    'Enquête publique — Bir Gandouz': 'بحث عمومي — بئر كندوز',
    'Lancement des programmes de développement': 'إطلاق برامج التنمية',
    'Avis d’appel d’offres et annonces': 'إعلانات طلبات العروض والإشعارات',
    'Galerie multimédia': 'معرض الوسائط',
    'Plan du site': 'خريطة الموقع',
  };
  return map[label] || label;
}

function arabicMeta(data) {
  const breadcrumb = (data.breadcrumb || []).map(([label, href]) => {
    if (!href) return [toArabicLabel(label)];
    if (href === '../index.html') return ['الرئيسية', 'index.html'];
    return [toArabicLabel(label), href.replace('../', '')];
  });
  return {
    title: `النسخة العربية · ${data.title}`,
    tag: toArabicLabel(data.tag),
    heading: toArabicLabel(data.heading),
    breadcrumb,
  };
}

function localizeArabicBody(bodyHtml) {
  const replacements = [
    ...
    ['>Télécharger<', '>تحميل<'],
    ['>Fiche PDF<', '>بطاقة PDF<'],
    ['>Cartographie (Taamir)<', '>الخرائط (Taamir)<'],
    ['>Pré-instruction en ligne<', '>الدراسة القبلية الإلكترونية<'],
    ['>Demande d\'informations<', '>طلب المعلومات<'],
    ['>Réclamation en ligne<', '>الشكايات الإلكترونية<'],
    ['>Bureau d\'Ordre Digital<', '>مكتب الضبط الرقمي<'],
    ...
    ['J’ai lu et j’accepte les conditions relatives au traitement de mes données personnelles.', 'لقد قرأت وأوافق على شروط معالجة بياناتي الشخصية.'],
    ...
    ['Cartographie (Taamir)', 'الخرائط (Taamir)'],
    ['Lire la suite', 'اقرأ المزيد'],
    ['Documents téléchargeables', 'وثائق قابلة للتحميل'],
    ['Contenu conforme à la fiche PDF', 'محتوى مطابق لبطاقة PDF'],
    ['Voir le catalogue', 'عرض الفهرس'],
    ['Procédures d’autorisation', 'مساطر الترخيص'],
...
${navHtml(assetPrefix, `ar/${filename}`)}
${mobileHtml(assetPrefix, `ar/${filename}`)}
...
function renderPageArabic(filename, data, assetPrefix) {
  const meta = arabicMeta(data);
  const bodyHtml = localizeArabicBody(data.body)
    .replaceAll('src="../assets/img/', 'src="../../assets/img/')
    .replaceAll("src='../assets/img/", "src='../../assets/img/")
  ...
  const containerClass = data.wide ? 'container content-prose' : 'container narrow content-prose';
  const bodyClass =
    'page-inner page-arabic' + (/^reg-/.test(filename) ? ' page-reglementation' : '');
  return `<!DOCTYPE html>
...
<body class="${bodyClass}">
${navHtmlArabic(assetPrefix, '../index.html')}
${mobileHtmlArabic(assetPrefix, '../index.html')}
<section class="page-hero">
  <div class="hero-bg"></div>
  <div class="hero-pattern"></div>
  <div class="container page-hero-inner">
    ${breadcrumbHtml(meta.breadcrumb, 'مسار التصفح')}
    <div class="section-tag">${meta.tag}</div>
...
${footerHtmlArabic(assetPrefix)}
<script src="${assetPrefix}assets/js/main.js"></script>
<script src="${assetPrefix}assets/js/forms-local.js"></script>
<script src="${assetPrefix}assets/js/arabic-link-localizer.js"></script>
${(data.extraScripts || []).map((s) => `<script src="${assetPrefix}${s}"></script>`).join('\n')}
${(data.extraModuleScripts || []).map((s) => `<script type="module" src="${assetPrefix}${s}"></script>`).join('\n')}
</body>
</html>`;
...
<body class="page-inner page-arabic">
${navHtmlArabic(assetPrefix, 'index.html')}
${mobileHtmlArabic(assetPrefix, 'index.html')}
<section class="page-hero">
  <div class="hero-bg"></div>
  <div class="hero-pattern"></div>
  <div class="container page-hero-inner">
    ${breadcrumbHtml(P['version-arabe.html'].breadcrumb, 'مسار التصفح')}
...
for (const [filename, data] of Object.entries(P)) {
  if (filename === 'version-arabe.html') {
    ...
    fs.mkdirSync(path.join(root, 'pages', 'ar'), { recursive: true });
    fs.writeFileSync(path.join(root, 'pages', 'ar', filename), renderPageArabic(filename, data, '../../'), 'utf8');
  }
}
